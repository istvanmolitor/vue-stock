...
import type { Warehouse } from '@stock/services/warehouseService'
...
const fetchCreateData = async () => {
  try {
    ...
    const { data } = await warehouseRegionService.getCreateData()
...
const handleSubmit = async () => {
  try {
    ...
    const response: any = await warehouseRegionService.create(form)
    toastService.success('A telephely régió sikeresen létrejött!')
    ...
    if (createdRegionId !== undefined && createdRegionId !== null) {
      await router.push({
        ...
        params: { id: String(createdRegionId) },
      })
...
<template>
  <AdminLayout pageTitle="Új régió">
    ...
    <Card v-else>
      <CardHeader>
        ...
        <CardDescription>Add meg az új régió adatait a létrehozáshoz.</CardDescription>
      </CardHeader>
